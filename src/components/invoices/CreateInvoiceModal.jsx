...
import { X, Plus, Trash2, FileText, User, Calendar, Package, DollarSign, StickyNote, UserCheck, CreditCard, MapPin } from 'lucide-react';
...
const QUOTES = ['QT-100201', 'QT-100202', 'QT-100203', 'QT-100204', 'QT-100205'];
...
export default function CreateInvoiceModal({ isOpen, onClose, onSave, invoiceToEdit }) {
  ...
  useEffect(() => {
    if (isOpen) {
      if (invoiceToEdit) {
        setForm({ ...invoiceToEdit, items: invoiceToEdit.items.map(i => ({ ...i })) });
      } else {
        const invNum = `INV-${Date.now().toString().slice(-6)}`;
        const due = new Date(Date.now() + 30 * 86400000).toISOString().split('T')[0];
        setForm(f => ({ ...f, invoiceNumber: invNum, dueDate: due, invoiceDate: new Date().toISOString().split('T')[0], items: [emptyItem()], contactName: '', companyName: '', billingAddress: '', relatedDeal: '', relatedQuote: '', taxPercent: 18, discountPercent: 0, amountPaid: 0, paymentMethod: '', terms: 'Payment is due within 30 days. Late payments may incur a 2% monthly fee.', remarks: '', owner: 'John Sales' }));
      }
      setErrors({});
    }
  ...
  const balance = useMemo(() => Math.max(0, grandTotal - form.amountPaid), [grandTotal, form.amountPaid]);
  ...
  const handleSave = () => {
    if (!validate()) return;
    const status = form.amountPaid >= grandTotal && grandTotal > 0 ? 'Paid' : form.status || 'Draft';
    onSave({
      ...form, id: form.id || `inv-${Date.now()}`,
      subtotal, taxAmount, discountAmount, totalAmount: grandTotal, balance,
      status, createdAt: form.createdAt || new Date().toISOString().split('T')[0],
    });
    onClose();
  };
    ...
    <AnimatePresence>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
        ...
        <motion.div initial={{ scale: 0.95, y: 16 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95 }}
          ...
          style={{ background: 'var(--card-bg)', borderRadius: 14, width: '100%', maxWidth: 860, maxHeight: '93vh', display: 'flex', flexDirection: 'column', boxShadow: '0 24px 64px rgba(0,0,0,0.25)', border: '1px solid var(--card-border)', overflow: 'hidden' }}>
          ...
          <div style={{ padding: '16px 24px', borderBottom: '1px solid var(--card-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div style={{ width: 36, height: 36, borderRadius: 10, background: 'rgba(16,185,129,0.12)', display: 'grid', placeItems: 'center' }}>
                <FileText size={18} color="#10b981" />
              </div>
              <div>
                <h2 style={{ fontSize: 16, fontWeight: 800, color: 'var(--text-primary)', margin: 0 }}>{invoiceToEdit ? 'Edit Invoice' : 'Create New Invoice'}</h2>
                <span style={{ fontSize: 12, color: 'var(--text-muted)', fontWeight: 600, fontFamily: 'monospace' }}>{form.invoiceNumber}</span>
              </div>
            </div>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 22 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              ...
              <div className="b24-field" style={{ marginBottom: 0 }}>
                ...
                <input className="b24-input" value={form.companyName} onChange={e => updateField('companyName', e.target.value)} placeholder="Company" />
              </div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
              <div className="b24-field" style={{ marginBottom: 0 }}>
                <label className="b24-label">Billing Address</label>
                <input className="b24-input" value={form.billingAddress} onChange={e => updateField('billingAddress', e.target.value)} placeholder="Street, City, State" />
              </div>
              <div className="b24-field" style={{ marginBottom: 0 }}>
              ...
              <div className="b24-field" style={{ marginBottom: 0 }}>
                <label className="b24-label">Related Quote</label>
                <select className="b24-select" value={form.relatedQuote} onChange={e => updateField('relatedQuote', e.target.value)}>
                  <option value="">None</option>
            ...
            {sectionTitle(<Calendar size={15} color="#10b981" />, 'Invoice Details')}
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
              <div className="b24-field" style={{ marginBottom: 0 }}>
                <label className="b24-label">Invoice Number</label>
                <input className="b24-input" value={form.invoiceNumber} readOnly style={{ background: 'var(--input-bg)', opacity: 0.7 }} />
              </div>
              <div className="b24-field" style={{ marginBottom: 0 }}>
                <label className="b24-label">Invoice Date</label>
                <input type="date" className="b24-input" value={form.invoiceDate} onChange={e => updateField('invoiceDate', e.target.value)} />
              </div>
              <div className="b24-field" style={{ marginBottom: 0 }}>
                <label className="b24-label">Due Date <span className="required">*</span></label>
                <input type="date" className={`b24-input ${errors.dueDate ? 'error' : ''}`} value={form.dueDate} onChange={e => updateField('dueDate', e.target.value)} />
                {errors.dueDate && <span className="b24-error">{errors.dueDate}</span>}
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              {/* Left: Tax, Discount, Payment */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {sectionTitle(<DollarSign size={15} color="#10b981" />, 'Tax & Discount')}
                ...
                {sectionTitle(<CreditCard size={15} color="#8b5cf6" />, 'Payment')}
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                  <div className="b24-field" style={{ marginBottom: 0 }}>
                    <label className="b24-label">Amount Paid (₹)</label>
                    <input type="number" min="0" className="b24-input" value={form.amountPaid} onChange={e => updateField('amountPaid', Math.max(0, Number(e.target.value)))} />
                  </div>
                  <div className="b24-field" style={{ marginBottom: 0 }}>
                    <label className="b24-label">Payment Method</label>
                    <select className="b24-select" value={form.paymentMethod} onChange={e => updateField('paymentMethod', e.target.value)}>
              ...
              {/* Right: Summary Card */}
              <div>
                {sectionTitle(<DollarSign size={15} color="#10b981" />, 'Invoice Summary')}
                <div style={{ background: 'var(--input-bg)', borderRadius: 10, border: '1px solid var(--card-border)', padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 10 }}>
                  ...
                  <div style={{ borderTop: '1px solid var(--card-border)', paddingTop: 8, marginTop: 4 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
                      <span style={{ color: 'var(--text-secondary)' }}>Amount Paid</span>
                      <span style={{ fontWeight: 700, color: '#2563eb' }}>{fmt(form.amountPaid)}</span>
                    </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              ...
              <div className="b24-field" style={{ marginBottom: 0 }}>
                <label className="b24-label">Remarks</label>
                <textarea className="b24-textarea" value={form.remarks} onChange={e => updateField('remarks', e.target.value)} placeholder="Internal remarks..." rows={3} />
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              <div className="b24-field" style={{ marginBottom: 0 }}>
                <label className="b24-label">Invoice Owner</label>
          ...
          <div style={{ padding: '14px 24px', borderTop: '1px solid var(--card-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0, background: 'var(--input-bg)' }}>
            <div>
              <div style={{ fontSize: 14, fontWeight: 800, color: '#10b981' }}>Total: {fmt(grandTotal)}</div>
              {balance > 0 && <div style={{ fontSize: 11, fontWeight: 600, color: '#ef4444' }}>Balance: {fmt(balance)}</div>}
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              <button className="b24-btn b24-btn-secondary" onClick={onClose}>Cancel</button>
              <button className="b24-btn b24-btn-primary" onClick={handleSave} style={{ background: '#10b981' }}>
                <FileText size={14} /> {invoiceToEdit ? 'Update Invoice' : 'Create Invoice'}
              </button>
            </div>
          </div>
        </motion.div>
